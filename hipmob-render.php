...
class HipmobWindow
{
  ...
  public function display()
  {
    ...
    if(!$app_id) return;
    $res = '<script type="text/javascript">var _hmc = _hmc || [];_hmc.push([\'app\', \''.esc_js($app_id).'\']);';
    
    // and, start building the bits: title
    $opt = get_option('hipmob_title');
    if($opt) $res .= '_hmc.push([\'title\', \''.esc_js($opt).'\']);';
    else $res .= '_hmc.push([\'title\', \'Talk to us\']);';
    
    // width/height
    $settings = array();
    $opt = get_option('hipmob_window_width'); if($opt) $settings['width'] = intval($opt);
    $opt = get_option('hipmob_window_height'); if($opt) $settings['height'] = intval($opt);
    
    // position
    $opt = get_option('hipmob_tab_position'); if($opt) $settings['position'] = $opt;
    if(function_exists('json_encode')) $res .= "_hmc.push(['settings', ".json_encode($settings)."]);";
    
    // tab
    $settings = array();
    $opt = get_option('hipmob_tab_background_color'); if($opt) $settings['background-color'] = $opt;
    $opt = get_option('hipmob_tab_text_color'); if($opt) $settings['color'] = $opt;
